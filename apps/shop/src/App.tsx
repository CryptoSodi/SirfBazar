      ...
      <aside className="flex w-56 shrink-0 flex-col bg-slate-900 text-slate-200">
        <div className="flex items-center gap-2 px-5 py-5">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-400">Merchant panel</div>
          </div>
        </div>
        ...
        <div className="border-t border-slate-800 px-5 py-4 text-xs">
          <div className="truncate font-medium text-white">{user?.fullName ?? user?.phoneNumber ?? user?.email}</div>
          <div className="text-slate-400">{user?.merchant?.shopName ?? user?.role?.replace(/_/g, ' ').toLowerCase()}</div>
